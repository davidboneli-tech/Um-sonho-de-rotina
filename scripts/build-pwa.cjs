...
html = html.replace('</head>', `<style>html,body,#root{width:100%;max-width:100%;overflow-x:hidden;touch-action:pan-x pan-y}html,body,body *{overscroll-behavior:none!important}</style><script>document.addEventListener('gesturestart',function(event){event.preventDefault();},{passive:false});document.addEventListener('gesturechange',function(event){event.preventDefault();},{passive:false});</script></head>`);
