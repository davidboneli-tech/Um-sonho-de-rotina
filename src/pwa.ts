...
export async function startPwa() {
  ...
  try {
    ...
    emit({ready: !!registration.active, update: !!registration.waiting});
    registration.addEventListener('updatefound', () => {
      const worker = registration!.installing;
      worker?.addEventListener('statechange', () => {
        if(worker.state === 'installed') emit({update: !!registration!.waiting});
...
export async function checkUpdate() { await registration?.update(); }
